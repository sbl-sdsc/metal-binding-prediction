...
class FOFEGenerator(keras.utils.Sequence):
    ...
    'Gererate data for Keras'
    def __init__(self,sequences,labels,translator,batch_size = 100,input_shape=(800,),label_shape=(706,),shuffle=True):
        'Initialization'
        self.sequences = sequences
        self.labels = labels
        self.translator = translator
    ...
    def generate_data(self, sequences_temp, labels_temp):
        'Populate input and label tensors'
        X = np.zeros((self.batch_size, *self.input_shape))
        y = np.zeros((self.batch_size, 706), dtype=int)
        ...
        features = fofe.naive_transform(sequences_temp, self.translator)
...
class Trainer():
    def __init__(self,
        ...
        #print (self.train_gen.label_shape)
